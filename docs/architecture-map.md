...
- **`Database/PlaceQueries.swift`, `RediscoveryQueries.swift`, `StackStore.swift`** —
  nonisolated `db`-taking query enums (the `NoteStore` shape).
- **`Models/PlacesStore.swift`, `RediscoveryStore.swift`, `StacksStore.swift`,
  `AppState+Places.swift`, `AppState+Rediscovery.swift`** — Pattern B stores + their
  methods-only AppState extensions.
- **`Components/StackDisplay.swift`** — pure collapse math.
- **`Intelligence/Stacks/`** — `BurstClusterer` (time-bucket-then-similarity),
