...
### Components added 2026-07-31 (Spec 02, photo library core)

- **`Intelligence/Core/FeaturePrints.swift`** — raw `VNFeaturePrintObservation.data`
  float-buffer parse + vDSP Euclidean distance. The fix for the shipped-dead
  visual-duplicates path; also the similarity primitive `BurstClusterer` uses.
- **`Filesystem/PhotoHeaderReader.swift`** — one `CGImageSourceCopyPropertiesAtIndex`
  read serving BOTH coordinates and EXIF (`PhotoHeader`/`ExifFields`). Supersedes
  Spec 01's `CoordinateReader` (deleted).
- **`Intelligence/PhotoHeaderBackfill.swift`** — launch pass, stale-by-either-marker
  selection, 5 000/launch, 200/transaction, 4-wide. Supersedes `CoordinateBackfill`
  (deleted). Chains `GeocodeBackfill` + `SearchFacets.refresh()` when it writes.
- **`Intelligence/Geo/`** — `GeoNamesDataset` (bundled `cities1000` extract, bounded
  inflate, uncached), `GeoKDTree` + `GreatCircle` + `GeoBounds` (pure geo math),
  `GeocodeBackfill` (holds `ReverseGeocoder` too).
- **`Resources/geonames-cities.tsv.zlib` / `geonames-admin1.tsv`** — bundled dataset,
  regenerated by `scripts/make-geonames.sh` (owner-only; currently placeholders).
- **`Search/`** — `SearchToken` (model + `SearchQueryParser` + `ParsedQuery`),
  `PhotoSearch` (token → SQL, indexed-only), `SearchFacets` (+ pure `SearchSuggest`).
- **REMOVED 2026-08-01 (owner decision):** the LIBRARY sidebar section (Places /
  On This Day / Rarely Seen / Shuffle) and near-duplicate stacks. Gone with them:
  `PlaceQueries`, `RediscoveryQueries`, `StackStore`, `PlacesStore`,
  `RediscoveryStore`, `StacksStore`, `AppState+Places`, `AppState+Rediscovery`,
  `StackDisplay`, `Intelligence/Stacks/` (`BurstClusterer`, `AutoStacker`),
  `PlacesPage`, `RediscoveryHeader`, `Sidebar/LibraryRows`. The geocoding data
  underneath Places STAYS — `near:`/`in:` search, the `.location` smart rule, the
  search facets and the Info card all still read the `places` table. Migrations
  v15–v17 stay too (the chain is append-only); their `views`/`stacks`/
  `stack_members` tables are simply unused now. `SeededRandom` moved to
  `Components/` (the geo k-d tree test still uses it).
