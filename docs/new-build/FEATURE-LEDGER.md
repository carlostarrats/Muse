...
| # | Gap | Status |
|---|---|---|
| G1 | **Almost nothing on `new-product-build-1` has been driven in the running GUI.** | **⚠️ THE AUTOMATED HALF IS DOWN as of 2026-08-04 (round 16).** 23 of the drive tests fail, and the app is NOT the reason: launched directly and queried through `CGWindowListCopyWindowInfo` it has exactly one normal on-screen window (1489×922, layer 0), and `MuseTagChipRowTests` passes in the same run reading 258 real buttons out of it. Three layers of harness/platform disagreement, each verified by probe and each present at the PREVIOUS commit too: (1) the window is now published with the AXDialog subrole, so `app.windows` matched nothing — **fixed** (`MainWindowFinder`), `testAppLaunchesWithPopulatedSidebar` back to green in 11s; (2) every grid tile reports `isHittable == false`, including ones in plain view, so the tile finder returned none — **fixed** (`GridTileFinder` now tests the tile's centre against the window, which is what `hit()` actually needs); (3) the double-click through a tile's coordinate no longer opens the hero viewer — **OPEN**, and it gates most of the remaining tests. Someone has to decide whether to keep investing in this harness or to rely on manual GUI verification; until then the automated regression net does not exist and a green suite must not be claimed. Historical status below. **PARTIALLY CLOSED 2026-08-01** (round 7) — `MuseSurfaceDriveTests` drives the app via XCUITest and confirms, by screenshot, that the editor, readouts, compare, duplicates, all five import panels, backup and settings all open and respond, and that every modal honours Escape. `MuseExportDriveTests` covers the export card the same way. **2026-08-02:** 12/12 green after the editor-canvas refactor — which is the runtime confirmation of that refactor, side-by-side included. (Six of those tests had been failing on a fixed window fraction, not on app behaviour; see the drive-suite rule in REVIEW-LENSES.) **Still open:** feature *correctness* (no slider moved, no render compared), a full export-to-disk run, social export, Drive publish, restore/delete, and running an import to completion. See the G1 section at the end of this file. |
...
| P33.3b | Customize card sizing | — | 2026-08-03 | ✅ `testCustomizeCardDoesNotClipItsLastRow` | **Owner caught this on screen.** The card shipped with NO padding, so the title sat against the top edge and the last row was clipped by the bottom — the presenter sizes a card to whatever its content reports, and a card with no padding asks to be exactly as tall as its rows. `.padding(28)` like every other sheet, width 420 → 460 to match `ImageLayoutSheet`. Now checked at the default AND the minimum window height. **Scrolling is deliberately NOT asserted**: the cap at the 480pt minimum window is 432pt and this content is ~411, so the card can never overflow — a first attempt asserted a scroll and passed while proving nothing, which is the vacuous pass this suite exists to avoid. Add a real scroll check if the list ever passes ~13 rows |
...
| P33.10 | Column-aware canvas insets | `EditorCanvasGeometryTests` (23, 9 new) | 2026-08-03 (round 12) | ⚠️ layout ✅, motion ❌ | **The glide was BROKEN and is now real (round 12).** The re-fit stepped `chromeProgress`, which had already settled by the time a column emptied, so thirteen frames recomputed an identical inset and the photo snapped — while the code said otherwise. The emptiedness is continuous now (`panelInsets(leftEmptied:rightEmptied:)`, 0…1) and that is what gets walked; a halfway value gives back half the space, pinned by test. **Whether it LOOKS smooth is still unseen** — a transition is not assertable from the element tree |
| P33.12 | Reorder without a mouse | `EditorWorkspaceTests` move-by-one arithmetic (3) | 2026-08-03 (round 12) | ❌ | **Round 12 finding.** The mode was drag-only and its VoiceOver hint promised a gesture such a user cannot make. Move Up / Move Down / Move to Other Column are now `accessibilityAction`s on the same model calls the drag uses, so the two routes cannot drift. **Unverified with VoiceOver actually running** — the arithmetic is tested, the rotor is not |
