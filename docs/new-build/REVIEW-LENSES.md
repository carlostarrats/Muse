...
| **One wire limit implemented in two language runtimes** | **17 (2026-08-05)** | 1 finding — Swift `String.count` measured grapheme clusters while JavaScript `String.length` measured UTF-16 units, so an emoji-heavy title passed publish and then made its own page unavailable. Swift now uses `utf16.count`; the boundary is negative-tested. |
| **A remote write whose RESPONSE can fail after its COMMIT** | **17** | 1 finding — a portfolio manifest PATCH error rolled back new images even though Drive may have committed the PATCH and lost only its response, leaving the live manifest pointed at deleted files. Once PATCH starts, both sets are retained; a later successful update sweeps the unused one. |
| **Fixed per-container overhead after adding one more child** | **17** | 1 finding — the 1,000-image cap once fit the one-page assumption only in comments; adding `layout.json` made a full folder 1,002 children. `listChildren` now follows every `nextPageToken` and treats a list failure as a sweep warning. |
| **A displayed expiry DATE treated as an instant** | **17** | 1 finding — the page stayed live through its displayed day while the app sweeper compared the record's incidental time directly and could delete the folder earlier. Both now cut over at the start of the following local day; impossible calendar dates also fail closed. |
