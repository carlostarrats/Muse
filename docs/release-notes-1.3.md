...
- Security hardening on the Drive share path (token, keychain, CSP, and partial-folder-cleanup review).
- About modal refreshed to cover collection sharing, with a corrected privacy line.
- The Manage Drive Shares list prunes stale rows when a share's folder is gone, and won't orphan a share if a delete fails.
