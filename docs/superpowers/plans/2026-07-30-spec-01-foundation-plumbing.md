...
roadmap: doctrine + build-setting updates, Sparkle excision (Mac App Store move), a v13
GPS-coordinates migration, the three edit-aware seams (thumbnail cache key, layout
geometry, export choke point) that make Spec 04's editor a 3–5 week job instead of a
rewrite, StoreKit 2 commercial plumbing (unenforced trial gate), the announcements
channel, and a performance-baseline harness.
...
target after this spec: **one** (GRDB) — Sparkle is removed.
...
- **No EXIF/XMP writes.** All new metadata (coordinates) is DB-only, read-only against
  source files.
- **Network policy after this spec: three app-initiated paths.** (1) Drive share
  (user-initiated, unchanged), (2) `announcements.json` (launch, off-able, this spec),
  (3) custom-domain provisioning Worker (future spec, not built here). StoreKit traffic
  is OS-level. Sparkle's feed fetch is removed entirely.
- **Dependencies stay at one (GRDB) after Sparkle removal.** No new third-party package
  is added anywhere in this spec.
...
## Section A — Doctrine, build settings, Sparkle excision
...
**Files:**
- Modify: `CLAUDE.md` (flip the phase-table row added in Task 1 Step 4 from 🚧 to ✅;
  update the "Implementation status" summary line if one exists)
...
- [ ] **Step 3: Append a `docs/session-log.md` entry**
  ...
  Dated `2026-07-30` (or the actual merge date), summarizing: doctrine revisions,
  Sparkle removal + MAS build config, v13 coordinates migration + backfill, the three
  edit-aware seams (why they matter for Spec 04), StoreKit 2 plumbing (unenforced trial
  gate — note explicitly WHY it's unenforced), the announcements channel, the perf
  baseline harness, and the semantic-search cancellation fix. Match the length and tone
  of recent entries (e.g. the Polish 25-28 entries) — a few paragraphs, not a full
  restatement of this plan.

- [ ] **Step 4: Flip the `CLAUDE.md` phase-table row to ✅**

  From Task 1 Step 4, change the status column to `✅ shipped` and fill in the actual
  branch name.
