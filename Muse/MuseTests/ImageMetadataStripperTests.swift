...
final class ImageMetadataStripperTests: XCTestCase {
    ...
    func testStripMultiFrameMetadataStripped() throws {
        ...
        guard raw.range(of: Data(needlePage.utf8)) != nil,
              raw.range(of: Data(needleContainer.utf8)) != nil else {
            throw XCTSkip("host did not embed TIFF page/container metadata; nothing to assert")
        }
        let out = try ImageMetadataStripper.strip(url: url, mime: "image/tiff")
