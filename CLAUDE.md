...
| Polish 17 — iCloud collection share (parallel branch off main; see its own spec/session log) | ✅ built, unmerged | `feat/icloud-collection-share` |
| Polish 18 — Google Drive collection share (backend #2, the "magic" path): "Share Drive Link" → OAuth `drive.file`/PKCE sign-in → upload images + print PDF into a tidy `My Drive/Muse/<collection> — <date>/` → link-share → branded Cloudflare page (manifest in the URL fragment, images from Drive thumbnails). View-menu "Manage Drive Shares…" (open link / unpublish). Muse-local expiry sweep deletes the folder past its date. **First sanctioned network egress beyond Sparkle** — opt-in, user-initiated. Pure units (PKCE/manifest/store/expiry/multipart) + JS tests; OAuth/Drive/page integration-only (needs the owner's OAuth client + Cloudflare domain). | ✅ built, unmerged | `feat/drive-collection-share` |
