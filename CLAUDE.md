...
- **The async (Color/Shape) `resort()` publish must stay behind ALL of: `resortToken` + `folderLoadToken` + `collectionRequestToken` + `searchRequestToken`/`!isSearchActive`, AND publish the intersection with the live grid.** Those sorts read the DB off-main; a stale result landing after `runSearch` overwrites ranked results, and direct removals (burn-delete, hero delete, removeRoot) bump no token — only the intersection keeps a trashed tile from resurrecting. Additions always arrive via token-bumping paths (reload / onRestore→resort), so the filter can't drop them.
