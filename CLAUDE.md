...
| Polish 38 — **welcome onboarding + empty-library guidance** (three short pages covering Smart Collections, right-click collection/Compare actions, collection export/publishing and the Full Guide; detailed native UI demonstrations; automatic only for an unseen empty setup; Help can reopen it; one empty-state Add Folder action with the supplied Muse wordmark) | ✅ built + verified (2,206 XCTest cases, 2 skipped, 0 failed; Swift Testing smoke green; focused first-launch/relaunch XCUITest green; audit 21/21; French export translated; universal Release green); **owner-approved in the running app** after the final UI, motion, copy-layout and empty-state rounds | `feat/next-160` |

Welcome onboarding deliberately owns its lifecycle in a dedicated
`WelcomeOnboardingStore`, not another published `AppState` property. Selecting
the automatic welcome suppresses the announcement fetch for that launch. A
manual Help-menu presentation never changes the persisted seen state.
...
> were built 2026-07-31. (Migrations have since run on: the chain reaches **v24**
> — per-file identity, 2026-08-03, plus **v25** repairing two of its own defects — so the next one starts at v26.)
> **They have since been reviewed in six rounds** (all 2026-08-01):
...
> **Run `./scripts/audit-invariants.sh` (21 checks, all negative-tested) before
...
> per-file identity, overwrite-safe export writes, PhotoKit purpose string,
> no raw `Error` in an `NSLog`). It
...
**The full set of 200 rules lives in `docs/durable-constraints.md`. READ THE
...
- **Identity is the FILE ON DISK, and a `files` row must never hold two alive paths (2026-08-03).** `content_hash` was UNIQUE, so N byte-identical files collapsed onto one row and shared everything hanging off it — edit stack, tags, ratings, note, collection membership. The owner's repro: twelve `RAW_SONY_ILCA-77M2*.ARW` in one folder, twelve alive paths, ONE `edits` row, so editing one changed all twelve (and eleven of the twelve filenames were unfindable, since one FTS row holds one basename). Migration **v24** splits existing rows; `Indexer.reconcile` gives every new copy its own row and INHERITS the donor's edits/tags/note/manual memberships (`InheritDonor`: same folder → most recently edited → lowest path). A rename/move still carries its edits, via an explicit distinction — an **orphaned** row (holds these bytes, no alive path) is the same file renamed, so adopt it; a row still alive elsewhere means this is a COPY. Content stays a *grouping* key (Find Duplicates, analysis reuse) but is not identity: `AnalysisReuse.adopt` + `Indexer.inherit` mean identical bytes are Visioned ONCE. **The UNIQUE constraint was the structural guarantee and it is deliberately gone — SQLite cannot express "at most one alive path", so the tests ARE the invariant** (audit `PFI-1` fails if they're deleted; `PFI-2` fails if `content_hash` is made UNIQUE again). Deleted with it: the split-on-edit branch, the hash-collision carry, `unionTags`, `inheritVisionTags`, both `carryAll`s. Sidecars are `<hash>__<basename>.json` (the hash-only name collided for same-folder copies; the old name is still read). Spec: `docs/superpowers/specs/2026-08-03-per-file-identity-design.md`.
