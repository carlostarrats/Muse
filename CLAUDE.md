...
- **Hero open/close toolbar = `ToolbarFade` (AppKit alpha fade on the always-mounted toolbar), never `.toolbar(.hidden, for: .windowToolbar)` (2026-07-06).** The SwiftUI toggle tears down/re-materializes the native NSToolbar — that re-materialization was the "empty nav, icons pop in late" delay on close. And the close "flash" (2026-06-18, long misattributed to the toolbar and "accepted as inherent") was really the hero BACKDROP unmounting before its 0.4s fade-out finished (~1–2% residual vanishing in one frame, app-wide): its out-animation is **0.3s on purpose** (in stays 0.4s) and must stay under the 0.36s unmount — don't re-symmetrize. ToolbarFade's fade-in is an explicit fast-start `CABasicAnimation` (`NSAnimationContext`'s ease measured back-loaded → reads as empty nav); the lookup fails open (view not found → toolbar just stays visible). **Full-screen: `toolbarView()` bails on a `.fullScreen` window and both calls no-op — macOS relocates the NSToolbarView into a separate auto-hiding `NSToolbarFullScreenWindow` it owns; DON'T fade/hide that view (fighting the OS auto-hide, and it strands hidden on exit).** But the hidden alpha/`isHidden` DOES carry back to the windowed view across a full-screen round-trip (verified), so ToolbarFade tracks `lastIntentHidden` and re-asserts it on `didExitFullScreenNotification` — else a hero hidden windowed, then closed while full-screen (show() no-ops there), strands the toolbar invisible after exit. Don't remove that guard or the full-screen bail. **Escape must fire ONLY `viewerClosing = true`** and let `startClose()` own the whole close (incl. setting `viewerDismissing`), exactly as the X button does — a separate `viewerDismissing` write in the Escape path made Escape need TWO presses.
