...
- **iCloud share folder-name safety (DO NOT relax — data-loss + privacy).** The share folder name is derived from the collection name, and `copyMembers` does a clean-and-recopy (`removeItem(folder)` then recreate) — so the leaf name MUST be a safe, collision-free, single component. Three rules, each fixing a found bug: (1) `sanitizedFolderName` maps `/ \ :`→`-` AND rejects empty/`.`/`..`/all-dots (those survived the char map and made `appendingPathComponent("..")` escape → `removeItem` would delete the whole iCloud Documents zone). (2) `uniqueFolderName(for:identity:owners:)` disambiguates collections that would share a leaf with `-2`/`-3` — covering BOTH names that sanitize alike (`Trip/Italy` vs `Trip-Italy`) AND two collections with an IDENTICAL display name. Ownership keys on the collection's STABLE id (`identity`), NOT its display name — else identical-named collections clobber/repoint each other (threaded `ShareCollectionButton`→`start(collectionID:)`→the record's optional `collectionID`). The same collection still reuses its folder (re-share = refresh). (3) `copyMembers` runs the destructive ops in a detached task chained on `inFlightCopy` (a detached task does NOT observe the service's cancellation) so a double-tap / re-share never races two copies on one folder. (4) EVERY destructive `removeItem` on a share folder goes through the shared pure guard `ICloudSharePaths.isContainedShareFolder(_:shareRoot:)` (leaf non-empty/`.`/`..` + parent == share root) — both `copyMembers` AND `ManageICloudSharesView.delete` (which trusts a path from the on-disk JSON store, so it MUST re-validate). Keep all four; the iCloud copy path is integration-only, so the pure pieces (`sanitizedFolderName`/`uniqueFolderName`/`isContainedShareFolder`) are the unit-tested safety net.
