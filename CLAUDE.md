...
| Polish 29 — **General image export** (one card, two preset families: Format — Same-as-original/JPEG/PNG/TIFF 8&16-bit/HEIC/WebP — above Spec 07's 12 Social presets, plus saved presets; quality, resize with never-upscale, EXIF/location toggles; `ExportPipeline` shared with `SocialRender`; real `.tiff16` at last; no filename controls and **no overwrite, ever**) | 🚧 built, **card not runtime-verified**; WebP pending `libwebp` | `feat/next-150` |
