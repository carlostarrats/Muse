...
| Polish 32 — **HDR gain maps** (one `HDRDecode` seam; 10-bit PQ HEIC thumbnail cache; headroom through the edit chain; headroom-aware histogram/clipping; byte-copy unedited exports + gain-map HEIC on macOS 15+; audit check HDR-1) | ✅ built + tested (1,985), **runtime OPEN** — needs a real gain-map HEIC on an EDR display, plan in `FEATURE-LEDGER.md` | `feat/next-153` |
