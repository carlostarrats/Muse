...
- **iCloud share folder-name safety (DO NOT relax — data-loss + privacy).** The share folder name is derived from the collection name, and `copyMembers` does a clean-and-recopy (`removeItem(folder)` then recreate) — so the leaf name MUST be a safe, collision-free, single component. Three rules, each fixing a found bug: (1) `sanitizedFolderName` maps `/ \ :`→`-` AND rejects empty/`.`/`..`/all-dots (those survived the char map and made `appendingPathComponent("..")` escape → `removeItem` would delete the whole iCloud Documents zone). (2) `uniqueFolderName(for:owners:)` disambiguates DIFFERENT collections that sanitize to the same leaf (`Trip/Italy` vs `Trip-Italy`) with `-2`/`-3` — otherwise sharing the second `removeItem`s the first's folder and silently repoints its already-distributed public link at the second's images. The same collection still reuses its folder (re-share = refresh). (3) `copyMembers` runs the destructive ops in a detached task chained on `inFlightCopy` (a detached task does NOT observe the service's cancellation) so a double-tap / re-share never races two copies on one folder; it also guards that `folder`'s parent == the share root before `removeItem` (backstops 1). Keep all three; the iCloud copy path is integration-only, so the pure pieces (`sanitizedFolderName`/`uniqueFolderName`) are the unit-tested safety net.
