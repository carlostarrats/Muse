...
| Polish 33 — **editor workspace** (the 12 cards become a persisted per-column ordered list + hidden set; View ▸ Editor Workspace ▸ Default Layout / Customize Modules / Reorder Modules; drag-reorder within and across columns reusing the sidebar's `ReorderMath`; **single column is NOT a mode** — it's the state where a column is empty; canvas insets follow the cards while the chrome row stays pinned; `EditorView` 1,682 → 656 lines) | ✅ built + tested (2,089), audit 15/15, **17 drive tests green** (12 existing + 5 new). Driving it found 2 bugs no unit test could: the Customize card opened BEHIND the editor (wrong stack), and Escape closed the whole viewer instead of cancelling the reorder (the hero's key monitor takes Escape ahead of SwiftUI). **The DRAG itself is still unverified — XCUITest can't drive a SwiftUI DragGesture**; see `FEATURE-LEDGER.md` Part 4 | `feat/next-155` |
